              ...
              {products.map((p) => {
                ...
                return (
                  <Table.Row key={p.id}>
                    <Table.Cell>
                      {p.thumbnail ? (
                        <img src={p.thumbnail} alt={p.title} className="w-12 h-12 rounded object-contain bg-slate-900 border border-slate-800" />
                      ) : (
                  ...
                  <div className="grid grid-cols-4 gap-4 mt-4">
                    {uploadedUrls.map((url, index) => (
                      <div key={index} className="relative group border border-slate-850 p-2 rounded bg-slate-900/50">
                        <img src={url} alt={`preview ${index}`} className="w-full h-24 object-contain" />
                        {index === 0 && (
